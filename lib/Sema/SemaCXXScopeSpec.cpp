...
NamedDecl *Sema::FindFirstQualifierInScope(Scope *S, NestedNameSpecifier *NNS) {
  ...
  if (NNS->getKind() != NestedNameSpecifier::DeclName)
    return nullptr;

  LookupResult Found(*this, NNS->getAsDeclName(), SourceLocation(),
...
bool Sema::BuildCXXNestedNameSpecifier(Scope *S, NestedNameSpecInfo &IdInfo,
                                       ...
                                       bool OnlyNamespace) {
  // This may happen after template argument substitution for declnames.
  if (!IdInfo.Identifier) {
    Diag(IdInfo.IdentifierLoc, diag::err_nested_name_spec_is_empty);
    return true;
  }
  ...
  if (Found.empty()) {
    if (isDependent &&
      ...
      SS.Extend(Context, Name, IdInfo.IdentifierLoc, IdInfo.CCLoc);
      return false;
    }
  }
