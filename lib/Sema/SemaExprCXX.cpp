...
Sema::BuildCXXTypeConstructExpr(TypeSourceInfo *TInfo,
  ...
  if (Exprs.size() == 1 && !ListInitialization &&
      !isa<InitListExpr>(Exprs[0]) && !isa<DesignatedInitExpr>(Exprs[0])) {
    Expr *Arg = Exprs[0];
...
Sema::BuildCXXNew(SourceRange Range, bool UseGlobal,
  ...
  if (!AllocType->isDependentType() &&
    ...
    if (FindAllocationFunctions(
            ...
            AllocType, ArraySize, PassAlignment, MappedPlacementArgs,
...
resolveAllocationOverload(Sema &S, LookupResult &R, SourceRange Range,
  ...
  bool HasDesig = S.AnyDesignated(Args);
  ...
  for (LookupResult::iterator Alloc = R.begin(), AllocEnd = R.end();
    ...
    if (FunctionTemplateDecl *FnTemplate = dyn_cast<FunctionTemplateDecl>(D)) {
      ...
      S.AddOverloadCandidate(Fn, Alloc.getPair(), Args, MappedArgs, Candidates,
                             HasDesig,
                             /*SuppressUserConversions=*/false);
    }
    MappedArgs.clear();
  ...
  auto CreateAllocationFunctionDecl = [&](Attr *ExtraAttr) {
    ...
    for (QualType T : Params) {
      ParamDecls.push_back(ParmVarDecl::Create(
          Context, Alloc, SourceLocation(), SourceLocation(), {}, T,
...
QualType Sema::CXXCheckConditionalOperands(ExprResult &Cond, ExprResult &LHS,
  ...
  if (!Context.hasSameType(LTy, RTy) &&
    ...
    unsigned RCF;
    if (CompareReferenceRelationship(QuestionLoc, LTy, RTy, RCF) ==
            Ref_Compatible &&
        !(RCF & (RCF_DerivedToBase | RCF_ObjCConversion |
                 RCF_ObjCLifetimeConversion)) &&
      ...
      RTy = RHS.get()->getType();
    } else if (CompareReferenceRelationship(QuestionLoc, RTy, LTy, RCF) ==
                   Ref_Compatible &&
               !(RCF & (RCF_DerivedToBase | RCF_ObjCConversion |
                        RCF_ObjCLifetimeConversion)) &&
