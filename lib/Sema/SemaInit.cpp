...
ExprResult Sema::ActOnDesignatedArgument(const Designator &D,
  ...
  DesignatedInitExpr *DIE = DesignatedInitExpr::Create(
      Context, Desig, None, Loc, false, Init.getAs<Expr>());
...
ResolveConstructorOverload(Sema &S, SourceLocation DeclLoc,
                           ...
                           bool HasDesig, bool SecondStepOfCopyInit = false) {
  CandidateSet.clear();
...
static void TryConstructorInitialization(Sema &S,
  ...
  if (IsListInit) {
    ...
    HasDesig = Sema::AnyDesignated(UnwrappedArgs);
...
static OverloadingResult TryRefInitWithConversionFunction(
  ...
  Sema::ReferenceCompareResult NewRefRelationship
    = S.CompareReferenceRelationship(DeclLoc, T1, cv3T3, NewRCF);
  ...
  if (NewRCF & Sema::RCF_DerivedToBase)
...
static void LookupCopyAndMoveConstructors(Sema &S,
  ...
  for (SmallVectorImpl<NamedDecl *>::iterator
    ...
    if (!Info.ConstructorTmpl) {
      ...
      S.AddOverloadCandidate(Info.Constructor, Info.FoundDecl,
                             CurInitExpr, NoMappedArgs, CandidateSet, /*HasDesig*/false);
      continue;
    }
    ...
    // candidates?
    S.AddTemplateOverloadCandidate(Info.ConstructorTmpl, Info.FoundDecl,
                                   nullptr, CurInitExpr, NoMappedArgs,
...
InitializationSequence::Perform(Sema &S,
  ...
  for (step_iterator Step = step_begin(), StepEnd = step_end();
    ...
    case SK_StdInitializerListConstructorCall: {
      ...
      MultiExprArg ExprArgs = Source ? MultiExprArg(Source) : Args;
      MultiExprArg SyntacticArgs;
      if (!CtorMappedArgs.empty()) {
        SyntacticArgs = ExprArgs;
        ExprArgs = CtorMappedArgs;
