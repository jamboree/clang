  ...
  if (Fn) {
    ...
    if (const FunctionProtoType *Proto =
      ...
      if (Proto->hasDesignators()) {
        FromType = QualType(Fn->getDesigProtoType(), 0);
        if (FromType.isNull())
          return false;
      }
    }
    ...
    if (FromType->isFunctionType()) {
      ...
      if (!S.Context.hasSameUnqualifiedType(UnqualToFunction, FromType)) {
        ...
        if (!S.IsFunctionConversion(FromType, UnqualToFunction, resultTy))
    ...
    if (Usable) {
      ...
      if (Info.ConstructorTmpl)
        S.AddTemplateOverloadCandidate(Info.ConstructorTmpl, Info.FoundDecl,
                                       /*ExplicitArgs*/ nullptr, From, NoMappedArgs, CandidateSet,
                                       /*HasDesig*/false,
                                       SuppressUserConversions);
      else
        S.AddOverloadCandidate(Info.Constructor, Info.FoundDecl, From,
                               NoMappedArgs, CandidateSet,
                               /*HasDesig*/false,
                               SuppressUserConversions);
    }
  }
        ...
        if (Usable) {
          ...
          if (Info.ConstructorTmpl)
            S.AddTemplateOverloadCandidate(
                ...
                /*ExplicitArgs*/ nullptr, Args, MappedArgs,
...
Sema::AddOverloadCandidate(FunctionDecl *Function,
  ...
  if (CXXMethodDecl *Method = dyn_cast<CXXMethodDecl>(Function)) {
    if (!isa<CXXConstructorDecl>(Method)) {
      ...
      AddMethodCandidate(Method, FoundDecl, Method->getParent(),
                         ...
                         SuppressUserConversions, PartialOverloading, EarlyConversions);
      return;
    }
  ...
  auto AddCandidate = [&, RawArgs = Args.size() ](unsigned NumArgs, bool Viable,
    ...
    Candidate.ExplicitCallArguments = RawArgs;
    return Candidate;
  };
  ...
  if (Constructor) {
    ...
    if (Args.size() == 1 && Constructor->isSpecializationCopyingObject() &&
        (Context.hasSameUnqualifiedType(ClassType, Args[0]->getType()) ||
         IsDerivedFrom(Args[0]->getLocStart(), Args[0]->getType(),
                       ClassType))) {
      AddCandidate(0, false).FailureKind = ovl_fail_illegal_constructor;
      return;
    }
    ...
    if (Shadow && Args.size() == 1 && Constructor->getNumParams() >= 1 &&
      ...
      if ((Context.hasSameUnqualifiedType(P, C) || IsDerivedFrom(Loc, P, C)) &&
        ...
        AddCandidate(0, false).FailureKind = ovl_fail_inhctor_slice;
        return;
      }
    }
  }
  ...
  auto Sink = [&](unsigned FailureKind, const DesignationFailureInfo &Info) {
    OverloadCandidate &Candidate = AddCandidate(0, false);
    Candidate.FailureKind = FailureKind;
    Candidate.DesignationFailure = Info;
    return FailureKind;
  };
  const FunctionProtoType *DesigProto = Function->getDesigProtoType();
  if (!DesigProto)
    DesigProto = Proto;
  if (DesignateArguments(DesigProto, NumParams, MinRequiredArgs, Args,
                         MappedArgs, Sink, HasDesig, PartialOverloading))
    return;

  // Add this candidate
  OverloadCandidate &Candidate = AddCandidate(Args.size(), true, MappedArgs);
  ...
  for (unsigned ArgIdx = 0; ArgIdx < Args.size(); ++ArgIdx) {
    if (Candidate.Conversions[ArgIdx].isInitialized()) {
      ...
      if (ArgIdx < MinRequiredArgs) {
        Candidate.Viable = false;
        Candidate.FailureKind = ovl_fail_missing_argument;
        Candidate.DesignationFailure = {nullptr, ArgIdx};
        return;
      }
    }
    ...
    if (FunctionDecl *FD = dyn_cast<FunctionDecl>(D)) {
      if (isa<CXXMethodDecl>(FD) && !cast<CXXMethodDecl>(FD)->isStatic())
        AddMethodCandidate(cast<CXXMethodDecl>(FD), F.getPair(),
                           ...
                           Args.slice(1), MappedArgs, CandidateSet, HasDesig,
                           SuppressUserConversions, PartialOverloading);
      else
        AddOverloadCandidate(FD, F.getPair(), Args, MappedArgs,
                             CandidateSet, HasDesig,
                             SuppressUserConversions, PartialOverloading);
    } else {
...
void Sema::AddMethodCandidate(DeclAccessPair FoundDecl,
  ...
  if (FunctionTemplateDecl *TD = dyn_cast<FunctionTemplateDecl>(Decl)) {
    ...
    AddMethodCandidate(cast<CXXMethodDecl>(Decl), FoundDecl, ActingContext,
                       ObjectType, ObjectClassification, Args, MappedArgs,
...
Sema::AddMethodCandidate(CXXMethodDecl *Method, DeclAccessPair FoundDecl,
  ...
  const FunctionProtoType *DesigProto = Method->getDesigProtoType();
  if (!DesigProto)
    DesigProto = Proto;
  if (DesignateArguments(DesigProto, NumParams, MinRequiredArgs, Args,
                         MappedArgs, Sink, HasDesig, PartialOverloading))
  ...
  OverloadCandidate &Candidate =
      AddCandidate(Args.size() + 1, true, MappedArgs);
  ...
  for (unsigned ArgIdx = 0; ArgIdx < Args.size(); ++ArgIdx) {
    if (Candidate.Conversions[ArgIdx + 1].isInitialized()) {
      ...
      if (Args[ArgIdx]) {
        ...
        // parameter of F.
        QualType ParamType = Proto->getParamType(ArgIdx);
        Candidate.Conversions[ArgIdx + 1] = TryCopyInitialization(
            *this, Args[ArgIdx], ParamType, SuppressUserConversions,
            /*InOverloadResolution=*/true,
            /*AllowObjCWritebackConversion=*/
            getLangOpts().ObjCAutoRefCount);
...
Sema::AddMethodTemplateCandidate(FunctionTemplateDecl *MethodTmpl,
  ...
  if (TemplateDeductionResult Result = DeduceTemplateArguments(
          MethodTmpl, ExplicitTemplateArgs, Args, MappedArgs,
          Specialization, Info, HasDesig, PartialOverloading,
  ...
  AddMethodCandidate(cast<CXXMethodDecl>(Specialization), FoundDecl,
                     ...
                     Args, MappedArgs, CandidateSet, false,
                     SuppressUserConversions, PartialOverloading,
                     Conversions);
}
...
Sema::AddTemplateOverloadCandidate(FunctionTemplateDecl *FunctionTemplate,
  ...
  ConversionSequenceList Conversions;
  if (TemplateDeductionResult Result = DeduceTemplateArguments(
          FunctionTemplate, ExplicitTemplateArgs, Args, MappedArgs,
          Specialization, Info, HasDesig, PartialOverloading,
          [&](ArrayRef<QualType> ParamTypes) {
  ...
  AddOverloadCandidate(Specialization, FoundDecl, Args,
                       MappedArgs, CandidateSet, false,
                       SuppressUserConversions, PartialOverloading,
...
void Sema::AddSurrogateCandidate(
    CXXConversionDecl *Conversion, DeclAccessPair FoundDecl,
    CXXRecordDecl *ActingContext, const FunctionProtoType *Proto, Expr *Object,
    ArrayRef<Expr *> Args, SmallVectorImpl<Expr *> &MappedArgs,
  ...
  if (DesignateArguments(Proto, NumParams, NumParams, Args, MappedArgs, Sink,
                         HasDesig, false))
    return;
...
void Sema::AddMemberOperatorCandidates(OverloadedOperatorKind Op,
  ...
  if (const RecordType *T1Rec = T1->getAs<RecordType>()) {
    ...
    bool HasDesig = AnyDesignated(Args.slice(1));
    SmallVector<Expr *, 32> MappedArgs;
    ...
    for (LookupResult::iterator Oper = Operators.begin(),
      ...
      AddMethodCandidate(Oper.getPair(), Args[0]->getType(),
                         Args[0]->Classify(Context), Args.slice(1), MappedArgs,
                         CandidateSet, HasDesig,
                         /*SuppressUserConversions=*/false);
      MappedArgs.clear();
    }
...
Sema::AddArgumentDependentLookupCandidates(DeclarationName Name,
  ...
  ArgumentDependentLookup(Name.getCanonicalName(), Loc, Args, Fns);
  ...
  for (ADLResult::iterator I = Fns.begin(), E = Fns.end(); I != E; ++I) {
    ...
    if (FunctionDecl *FD = dyn_cast<FunctionDecl>(*I)) {
      ...
      AddOverloadCandidate(FD, FoundDecl, Args, MappedArgs, CandidateSet,
                           HasDesig, false, PartialOverloading);
    } else
      AddTemplateOverloadCandidate(cast<FunctionTemplateDecl>(*I),
                                   FoundDecl, ExplicitTemplateArgs,
                                   Args, MappedArgs, CandidateSet,
                                   HasDesig, false, PartialOverloading);
    MappedArgs.clear();
  }
}

unsigned Sema::DesignateArguments(const FunctionProtoType *Proto,
                                  unsigned NumParams, unsigned MinRequiredArgs,
                                  ArrayRef<Expr *> &Args,
                                  SmallVectorImpl<Expr *> &MappedArgs,
                                  DesignationFailureSink Sink, bool HasDesig,
                                  bool PartialOverloading) {
  // We have processed the designated args previously, so just use it.
  if (!MappedArgs.empty()) {
    assert(!HasDesig && "should not have HasDesig in processed result");
    Args = MappedArgs;
    return 0;
  }
  ...
  if (TooManyArguments(NumParams, Args.size(), PartialOverloading) &&
      !Proto->isVariadic()) {
    return Sink(ovl_fail_too_many_arguments, {});
  }
  ...
  if (Args.size() < MinRequiredArgs && !PartialOverloading) {
    ...
    return Sink(ovl_fail_too_few_arguments, {});
  }
  ...
  class DesigParamFinder {
    ...
    QualType const *Params;
    unsigned Index;

  public:
    const unsigned End;

    explicit DesigParamFinder(QualType const *Params, unsigned NumParams)
        : Params(Params), Index(0), End(NumParams) {}
    ...
    unsigned Find(IdentifierInfo *Id) {
      ...
      for (; Index != End; ++Index) {
        ...
        if (const DesignatingType *Desig = Param->getAs<DesignatingType>()) {
          auto PId = Desig->getDesigName().getAsIdentifierInfo();
          if (Id == PId)
            return Index++;
          Cache[PId] = Index;
        }
  ...
  DesigParamFinder Finder(Proto->param_type_begin(), NumParams);
  ...
  for (auto Arg : Args) {
    ...
    if (MappedArgs[Index]) {
      // Conflict arguments
      return Sink(ovl_fail_overlapped_arguments, {Arg, Index});
    }
...
static void DiagnoseDesignatorNotFound(Sema &S, NamedDecl *Found,
                                       FunctionDecl *Fn,
  ...
  for (auto Param : Fn->parameters()) {
    if (Param->getIdentifier() == Id) {
      Loc = Param->getLocation();
      mode = 1;
      break;
    }
  }
  S.Diag(Loc, diag::note_ovl_designator_not_found)
      << (unsigned)FnKind << (Fn->getDescribedFunctionTemplate() != nullptr)
      << mode << Id;
}

static void DiagnoseBadArgumentPos(Sema &S, NamedDecl *Found, FunctionDecl *Fn,
                                   const DesignationFailureInfo &Info,
                                   unsigned mode) {
  std::string Description;
  OverloadCandidateKind FnKind =
...
static void DiagnoseBadDeduction(Sema &S, NamedDecl *Found, Decl *Templated,
  ...
  case Sema::TDK_TooManyArguments:
  case Sema::TDK_TooFewArguments:
    DiagnoseArityMismatch(S, Found, Templated, Args.size());
    return;
  ...
  case Sema::TDK_DesignatorNotFound:
    DiagnoseDesignatorNotFound(S, Found, dyn_cast<FunctionDecl>(Templated),
                               DeductionFailure.DesignationFailure);
    break;

  case Sema::TDK_OverlappedArguments:
    DiagnoseBadArgumentPos(S, Found, dyn_cast<FunctionDecl>(Templated),
                           DeductionFailure.DesignationFailure, 0);
    break;
  case Sema::TDK_MissingArgument:
    DiagnoseBadArgumentPos(S, Found, dyn_cast<FunctionDecl>(Templated),
...
static void DiagnoseBadDeduction(Sema &S, OverloadCandidate *Cand,
  ...
  if (TDK == Sema::TDK_TooFewArguments || TDK == Sema::TDK_TooManyArguments) {
    if (CheckArityMismatch(S, Cand, Args.size()))
      return;
  }
  DiagnoseBadDeduction(S, Cand->FoundDecl, Cand->Function, // pattern
                       Cand->DeductionFailure, Args, TakingCandidateAddress);
}
...
static void NoteFunctionCandidate(Sema &S, OverloadCandidate *Cand,
  ...
  case ovl_fail_too_few_arguments:
    return DiagnoseArityMismatch(S, Cand, Args.size());

  case ovl_fail_bad_deduction:
    return DiagnoseBadDeduction(S, Cand, Args,
  ...
  case ovl_fail_designator_not_found:
    DiagnoseDesignatorNotFound(S, Cand->FoundDecl, Cand->Function,
                               Cand->DesignationFailure);
    break;

  case ovl_fail_overlapped_arguments:
    DiagnoseBadArgumentPos(S, Cand->FoundDecl, Cand->Function,
                           Cand->DesignationFailure, 0);
    break;

  case ovl_fail_missing_argument:
    DiagnoseBadArgumentPos(S, Cand->FoundDecl, Cand->Function,
                           Cand->DesignationFailure, 1);
    break;
...
void OverloadCandidateSet::NoteCandidates(
  ...
  for (I = Cands.begin(), E = Cands.end(); I != E; ++I) {
    ...
    if (Cand->Function)
      NoteFunctionCandidate(S, Cand, Args,
...
void TemplateSpecCandidate::NoteDeductionFailure(Sema &S,
  ...
  DiagnoseBadDeduction(S, FoundDecl, Specialization, // pattern
                       DeductionFailure, ArrayRef<Expr *>(), ForTakingAddress);
}
...
private:
  bool candidateHasExactlyCorrectType(const FunctionDecl *FD) {
    ...
    if (const FunctionProtoType *Proto =
      ...
      if (Proto->hasDesignators()) {
        FT = QualType(FD->getDesigProtoType(), 0);
        if (FT.isNull())
          return false;
      }

    return Context.hasSameUnqualifiedType(TargetFunctionType, FT) ||
           S.IsFunctionConversion(FT, TargetFunctionType, Discard);
  }
...
static void AddOverloadedCallCandidate(Sema &S,
  ...
  if (FunctionDecl *Func = dyn_cast<FunctionDecl>(Callee)) {
    ...
    S.AddOverloadCandidate(Func, FoundDecl, Args, MappedArgs,
                           CandidateSet, HasDesig,
  ...
  if (FunctionTemplateDecl *FuncTemplate
    ...
    S.AddTemplateOverloadCandidate(FuncTemplate, FoundDecl,
                                   ExplicitTemplateArgs, Args, MappedArgs,
                                   CandidateSet, HasDesig,
                                   /*SuppressUsedConversions=*/false,
                                   PartialOverloading);
    return;
    ...
    if (!R.empty()) {
      ...
      for (LookupResult::iterator I = R.begin(), E = R.end(); I != E; ++I) {
        AddOverloadedCallCandidate(SemaRef, I.getPair(), ExplicitTemplateArgs,
                                   Args, MappedArgs, Candidates, HasDesig,
...
/// the given function.
/// \returns true when the ExprResult output parameter has been set.
...
Sema::BuildCallToMemberFunction(Scope *S, Expr *MemExprE,
  ...
  if (BinaryOperator *op = dyn_cast<BinaryOperator>(NakedMemExpr)) {
    ...
    if (DesignateArgumentsForCall(proto, op, Args, MappedArgs))
      return ExprError();
    if (!MappedArgs.empty()) {
      SyntacticArgs = Args;
      Args = MappedArgs;
    ...
    if (!SyntacticArgs.empty())
      call->setSyntacticArgs(Context, SyntacticArgs);
    ...
    for (UnresolvedMemberExpr::decls_iterator I = UnresExpr->decls_begin(),
      ...
      if (getLangOpts().MicrosoftExt && isa<CXXConstructorDecl>(Func)) {
        ...
        AddMethodCandidate(Method, I.getPair(), ActingDC, ObjectType,
                           ObjectClassification, Args, MappedArgs, CandidateSet,
                           HasDesig, /*SuppressUserConversions=*/false);
      } else {
...
bool Sema::DesignateArgumentsForCall(FunctionDeclOrProtoType Function,
                                     Expr *CallExpr, MultiExprArg Args,
                                     SmallVectorImpl<Expr *> &MappedArgs) {
  if (!AnyDesignated(Args))
    return false;

  const FunctionDecl *FD = Function.dyn_cast<const FunctionDecl *>();
  const FunctionProtoType *Proto = nullptr;
  unsigned MinRequiredArgs;
  if (FD) {
    MinRequiredArgs = FD->getMinRequiredArguments();
    Proto = FD->getType()->getAs<FunctionProtoType>();
    if (const FunctionProtoType *DesigProto = FD->getDesigProtoType())
      Proto = DesigProto;
  } else {
    Proto = Function.get<const FunctionProtoType *>();
    MinRequiredArgs = Proto->getNumParams();
  }
  if (!Proto)
    return false;
  ...
  auto Sink = [&](unsigned FailureKind, const DesignationFailureInfo &Info) {
    ...
    case ovl_fail_designator_not_found: {
      ...
      Diag(Info.Arg->getLocStart(), diag::err_protocheck_designator_not_found)
          << mode << Id << DIE->getDesignator(0)->getSourceRange();
      break;
    }
    case ovl_fail_argument_out_of_bounds:
      Diag(Info.Arg->getLocStart(), diag::err_protocheck_argument_out_of_bounds)
          << Info.ParamIndex << Proto->getNumParams()
          << Info.Arg->getSourceRange();
      break;
    case ovl_fail_overlapped_arguments:
      Diag(Info.Arg->getLocStart(), diag::err_protocheck_bad_argument_pos)
          << 0 << (Info.ParamIndex + 1) << Info.Arg->getSourceRange();
      break;
    default:
      return false;
    }
    if (FD)
      Diag(FD->getLocStart(), diag::note_callee_decl) << FD;
    return true;
  };

  if (DesignateArguments(Proto, Proto->getNumParams(), MinRequiredArgs,
                         Args, MappedArgs, Sink, true))
    return true;
  for (unsigned I = 0, E = Args.size(); I != E; ++I)
    if (!Args[I] && I < MinRequiredArgs) {
      Diag(CallExpr->getLocStart(), diag::err_protocheck_bad_argument_pos)
          << 1 << (I + 1) << CallExpr->getSourceRange();
      if (FD)
        Diag(FD->getLocStart(), diag::note_callee_decl) << FD;
      return true;
    }
    ...
    if (!Conv->isExplicit()) {
      ...
      if (const FunctionProtoType *Proto =
        ...
        AddSurrogateCandidate(Conv, I.getPair(), ActingContext, Proto,
                              Object.get(), Args, MappedArgs, CandidateSet,
                              HasDesig);
        MappedArgs.clear();
      }
    }
...
Sema::BuildOverloadedArrowExpr(Scope *S, Expr *Base, SourceLocation OpLoc,
  ...
  for (LookupResult::iterator Oper = R.begin(), OperEnd = R.end();
    ...
    AddMethodCandidate(Oper.getPair(), Base->getType(), Base->Classify(Context),
                       None, NoMappedArgs, CandidateSet, /*HasDesig*/false,
                       /*SuppressUserConversions=*/false);
  }
